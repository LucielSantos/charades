...
export function TeamList({ teams, onEdit, onDelete }: TeamListProps) {
	...
	return (
		<div className="space-y-3">
			{teams.map((team) => (
				<div key={team.id} className="flex items-center gap-3 rounded-xl bg-white p-4 shadow-sm">
					<div className="h-5 w-5 rounded-full shrink-0" style={{ backgroundColor: team.color }} />
					<span className="flex-1 font-semibold text-gray-800 truncate">{team.name}</span>
